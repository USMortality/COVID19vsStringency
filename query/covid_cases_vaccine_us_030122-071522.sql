...
FROM
    ...
    JOIN (
        ...
        WHERE
            str_to_date(date, '%m/%d/%Y') >= str_to_date('03/01/2022', '%m/%d/%Y')
            AND str_to_date(date, '%m/%d/%Y') < str_to_date('07/15/2022', '%m/%d/%Y')
            AND date_type = 'Admin'
        GROUP BY
            location
    ) c
